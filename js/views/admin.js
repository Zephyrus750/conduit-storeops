...
function migrTab(rec, c) {
  ...
  const r = st.importResult, src = st.importSource || 'k2b', dv = src === 'dv';
  const kv = !r ? '' : r.source === 'dv'
    ? `<span>DV site</span><b>${esc(r.legacy?.name || '—')} · ${esc(r.url)}${r.legacy?.storeNumber ? ' · store ' + esc(r.legacy.storeNumber) : ''}</b><span>Archive</span><b>${r.counts.history} decanted truck${r.counts.history === 1 ? '' : 's'}</b><span>On the dock</span><b>${r.counts.trucks} open truck${r.counts.trucks === 1 ? '' : 's'} · ${r.counts.pallets} pallets</b><span>Planner</span><b>${r.counts.planner} slot${r.counts.planner === 1 ? '' : 's'}</b>`
    : `<span>Legacy store</span><b>${esc(r.legacy?.name || '—')} · code ${esc(r.code)}${r.legacy?.storeNumber ? ' · number ' + esc(r.legacy.storeNumber) : ''}</b><span>History</span><b>${r.counts.history} bays on record</b><span>Today's board</span><b>${r.counts.today} bays · ${r.counts.requested} requested</b><span>Negative SOH</span><b>${r.counts.negsoh} items today</b>`;
  const result = !r ? '' : `<div class="card"><div class="ch"><h3>${r.dry ? 'Dry run' : 'Import'} · ${esc(r.legacy?.name || r.code || r.url)}</h3><span class="cs-dim">${r.dry ? 'nothing was written' : `${r.applied} events written · ${r.duplicates} already there`}</span></div><div class="ad-kv">${kv}<span>Events</span><b>${r.counts.events}${r.dry ? ' would be written' : ` · ${r.applied} written · ${r.duplicates} duplicates · ${r.rejected.length} rejected`}</b></div>` +
    ...
    (r.dry ? `<div class="acts" style="margin-top:12px"><button class="btn primary" data-act="import-accept">${ic('check')}Looks right · import now</button></div>` : `<div class="ad-done" style="margin:12px 0 0"><span class="ck">${ic('check')}</span><div><b>Imported.</b><span>${r.source === 'dv' ? 'Open the store\'s Receiving and Receiving history to check the dock and the archive, then flip the Back dock to live below. Decant Visualiser stays deployed; freezing it for this store is a manual step on that site.' : 'Open the store\'s Backfill review to check the board, then flip the Stockroom to live below. K2B stays deployed; freezing it for this store is a manual step in that app.'}</span></div></div>`) + `</div>`;
  const picker = `<div class="ad-field" style="margin-bottom:10px"><label>Legacy app</label><span class="pills"><button type="button" class="${dv ? '' : 'on'}" data-act="import-src" data-v="k2b">K2B · Stockroom</button><button type="button" class="${dv ? 'on' : ''}" data-act="import-src" data-v="dv">Decant Visualiser · Back dock</button></span></div>`;
  const fields = dv
    ? `<div class="ad-grid2">${field('Decant Visualiser site', inp('url', st.importUrl || '', 'https://busselton-dock.netlify.app', 'mono', 'required inputmode="url"'), 'the store’s DV address; its state is read without a code, so team members import as D-numbers')}</div>`
    : `<div class="ad-grid2">${field('K2B store code', inp('code', st.importCode || '', 'e.g. BUS247', 'mono', 'required autocapitalize="characters"'), 'the join code the store’s phones use')}${field('Store PIN in K2B', `<input class="ad-in mono" name="pin" type="password" autocomplete="off" required placeholder="••••">`, 'the review PIN; only its hash is stored, and only in K2B')}</div>`;
  const form = `<form class="card" data-form="import"><div class="ch"><h3>Import from ${dv ? 'Decant Visualiser' : 'K2B'}</h3><span class="cs-dim">${dv ? 'the archive, the trucks on the dock, the week plan' : 'history, today\'s board, requested bays, negative SOH'}</span></div>${picker}${fields}<div class="si-err" id="impErr"></div><div class="acts" style="margin-top:12px"><button class="btn primary" type="submit" data-dry="1">${ic('search')}Dry run</button><button class="btn" type="submit" data-dry="0">${ic('arrow')}Import</button></div><p class="lbl">A dry run reads everything and counts what would be written. Importing writes events with your owner id; running it again only adds what is new. ${dv ? (rec.entitlements?.backdock ? '' : '<b>Turn the Back dock on in Access first.</b>') : (rec.entitlements?.stockroom ? '' : '<b>Turn the Stockroom on in Access first.</b>')}</p></form>`;
...
async function runImport(ctx, form, dry) {
  ...
  const dv = (st.importSource || 'k2b') === 'dv';
  const code = dv ? '' : form.code.value.trim().toUpperCase(), pin = dv ? '' : form.pin.value, url = dv ? form.url.value.trim() : '';
  if (dv) st.importUrl = url; else st.importCode = code;
  btns.forEach(b => { b.disabled = true; }); err.textContent = ''; toast(dry ? (dv ? 'Reading the DV site…' : 'Reading the legacy worker…') : 'Importing…');
  try {
    st.importResult = await ctx.admin.api(`/v1/admin/stores/${st.no}/import`, { method: 'POST', body: dv ? { source: 'dv', url, dry } : { source: 'k2b', code, pin, dry }, timeoutMs: 300_000 });
    st.importPin = dry ? (dv ? 'dv' : pin) : null;
...
async function onStoreClick(e, ctx) {
  ...
  else if (act === 'import-accept') { const r = st.importResult; if (!r || !st.importPin) return toast('Run the dry run again first', 'bad'); const form = document.querySelector('form[data-form="import"]'); if (form) { if (r.source === 'dv') form.url.value = r.url; else { form.code.value = r.code; form.pin.value = st.importPin; } runImport(ctx, form, false); } }
  else if (act === 'import-src') { st.importSource = a.dataset.v; st.importResult = null; st.importPin = null; ctx.rerender(); }
}
