...
export default {
  ...
  desktop(ctx) {
    return vh('Emergency', sub('<span id="emsub">…</span>'), `<button class="btn primary" data-act="service">${ic('alert')}Log a service</button>`, 'm-emergency') +
  ...
  mount(ctx, root) {
    const map = mountMap($('#mapstage', root), { showEmergency: true, onSelect: info => { if (info.kind === 'marker') { selected = info.id; paint(); } } });
