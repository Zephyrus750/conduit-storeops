...
export default {
  ...
  desktop(ctx) {
    ...
    // The Stockroom and Back dock cards need that area's code on this device:
    // without it the data never reaches the device, so the card offers the code.
    const caps = ctx.session.current?.caps || [];
    const sr = caps.includes('stockroom') ? (hasArea(ctx.session, 'stockroom') ? stockroomCard(ctx) : lockedCard('Stockroom', 'box', 'bfreview')) : '';
    const bd = caps.includes('backdock') ? (hasArea(ctx.session, 'backdock') ? backdockCard(ctx) : lockedCard('Back dock', 'm-receiving', 'receiving')) : '';
