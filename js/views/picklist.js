...
export default {
  ...
  desktop(ctx) {
    ...
    return vh('Pick list', sub(`${m.items.length} items`, `${picked} picked`, m.items.length ? `${m.items.length - picked} stops to go` : ''), `<button class="btn" data-act="clear">${ic('trash')}Clear all</button>`, 'm-picklist') +
      `<div class="grid2"><div class="mapbox">${mapbar()}<div class="mapstage" id="mapstage"></div>` +
      `<div class="mapleg">${crumbx('Pick route', ctx.storeNo)}<span><i style="background:#16A34A"></i>Picked</span><span><i style="background:#D24E0E"></i>Next stops, in route order</span><span><i style="background:transparent;border:2px dashed #D24E0E"></i>Walk path</span><span style="color:var(--faint)">Tap a shelf to add it</span></div></div>` +
  ...
  mount(ctx, root) {
    ...
    const paint = () => {
      ...
      map.setMarks(marks); map.drawRoute(m.items.map(i => i.code), m.items.filter(i => i.completed).map(i => i.code));
    ...
    root.addEventListener('click', async e => {
      ...
      else if (act === 'add') { const inp = $('[data-field="add"]', root); const codes = inp.value.toUpperCase().split(/[\s,]+/).filter(Boolean); const bad = codes.filter(c => !map.groups(c).length); if (bad.length) toast(`Not in this store: ${bad.join(', ')}`, 'bad'); const ok = codes.filter(c => map.groups(c).length && !m.items.some(i => i.code === c)); if (ok.length) await save(ctx, [...m.items, ...ok.map(code => ({ code, completed: false }))]); inp.value = ''; }
